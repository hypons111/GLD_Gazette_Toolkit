...
///////////    Version 8    ///////////    2021-8-19    ///////////
//fixed english item tail contents applied sung font

///////////    Version 9    ///////////    2021-9-27    ///////////
//added makeHasHaveDialog() to choose has and have in english version 
...
function getHeader() {
    for(var x = 0; x < sectionNumbers.length; x ++) {
        ...
        if(result.length > 0) {
            if(sectionNumbers[x] === "747\\(7\\)") {
                makeHasHaveDialog(x)
            } else if(x > 8) {
                makeHeader(sections[x + 1])
            } else {
...
function makeHasHaveDialog(x) {
    var dialog = new Window('dialog', "", "x:0, y:0, width:200, height:75");

    var has = dialog.add('radiobutton', [20, 12.5, 90, 37.5], 'has')
    var have = dialog.add('radiobutton', [20, 45, 90, 60], 'have')
    has.value = true

    dialog.okButton = dialog.add('button', [110, 10, 185, 35], "OK");
    dialog.okButton.onClick = function() { dialog.close(1) }
    dialog.cancelButton = dialog.add('button', [110, 40, 185, 65], "Cancel");
