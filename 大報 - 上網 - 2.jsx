...
///////////    Version 10    ///////////    2021-8-2    ///////////
//made getExportPreset() returns preset directly 
//deleted presetList array

///////////    Version 11    ///////////    2021-8-5    ///////////
//fixed when getExportPreset() only returns '[大報]' when the file has only one GN

///////////    Version 12    ///////////    2021-8-18    ///////////
//fixed getExportPreset() returns '[大報有標籤]' at the last GN under unknown condition

///////////    Version 13    ///////////    2021-8-26    ///////////
//added new condition to match two constrution at GN header
...
for(var x = 0; x < activeDocument.pages.length; x ++) {
    for(var y = 0; y < activeDocument.pages[x].textFrames.length; y ++) {
        ...
        } else if(activeDocument.pages[x].textFrames[y].contents.toString().slice(2, 3) === "第") {
            getGnNumber(activeDocument.pages[x].textFrames[y].contents.toString().slice(3, 9))
        }
    } 
}
...
for(var x = 0; x < GNs.length; x ++) {
    ...
    activeDocument.exportFile(ExportFormat.pdfType, File('~/Desktop/Gaz_PDF/' + version + year + volume + issue + gnNumber[x] + '.pdf'), false, getExportPreset(GNs[x], GNs[x + 1]))
...
function getExportPreset(start, end) {
    if (typeof(end) === "undefined") {
        for(var z = Number(start - 1); z < activeDocument.pages.length; z ++) {
            for(var y = 0; y < activeDocument.pages[z].allGraphics.length; y ++) {
                if(activeDocument.pages[z].allGraphics[y].imageTypeName === "TIFF") {
                    z = activeDocument.pages.length
                    y = activeDocument.pages[z - 1].allGraphics.length
                    return app.pdfExportPresets.item('[大報有標籤]')
                }
            }
        }
    }

    for(var y = Number(start - 1); y < Number(end - 1); y ++) {
        for(var z = 0; z < activeDocument.pages[y].allGraphics.length; z ++) {
            if(activeDocument.pages[y].allGraphics[z].imageTypeName === "TIFF"){
                y = Number(end - 1)
                z = activeDocument.pages[y].allGraphics.length
                return app.pdfExportPresets.item('[大報有標籤]')
            }
        }
    }
    return app.pdfExportPresets.item('[大報]')
}
