...
if(isAuto()) {
    if(insertHeader(getHeader(version))) {
        ...
        orderListTextFile.close()
        orderListTextFile.rename(issue + '-list-' + new Date().getHours() + "_" + new Date().getMinutes() + '.txt')
    }
} else {
...
function insertHeader(t) {
    ...
    if(t.toString()[0] === "無") {
        ...
        currentTop = getNewestTop() - spacing
        return true
    }
    return false
}
...
function openFile() {
    try {
        ...
        try {
            app.open (File('//EXZIP18/Gazette_N/-Main Gazette--' + versionFullName + '/Testing' + version + issue + '/' + orderList[x] + '-' + version + issue + '_col.indd'));
            return app.activeDocument
        } catch(e) {
            alert("檔案名稱無效")
            return false
...
function isThreadTextFrame(doc) {
    if(doc.pages.length > doc.pages[0].textFrames[0].parentStory.textContainers.length
    || doc.pages.length < doc.pages[0].textFrames.length) {
...
function deleteLable(page) {
    for(var y = page.pageItems.length - 1; y > 0; y --) {
        if(page.pageItems[y].geometricBounds[0] > "665.717"
         || page.pageItems[y].geometricBounds[1] > "28"
         || page.pageItems[y].geometricBounds[2] < "0"
...
function applyPictureParagraphStyle(activeDocument) {
    for(var x = 0; x < activeDocument.pages.length; x ++) {
        for(var y = 0; y < activeDocument.pages[x].allGraphics.length; y ++) {
            if(activeDocument.pages[x].allGraphics[y].parent.parent.appliedParagraphStyle.name !== "Picture"
            && activeDocument.pages[x].allGraphics[y].parent.geometricBounds[1] === 0
            && Number(activeDocument.pages[x].allGraphics[y].parent.geometricBounds[3]) - Number(activeDocument.pages[x].allGraphics[y].parent.geometricBounds[1]) > 26) {
                activeDocument.pages[x].allGraphics[y].parent.parent.appliedParagraphStyle = "Picture"
            }
        }
    }
}
