...
const spacing = 45
var currentTop = getNewestTop(version)
const orderList = []
...
if(newIsAuto()) {
    ...
    var isContinue = true
    while(isContinue) {
        orderList[0] = Number(inputGNNumber())
        try{
...
function getNewestTop(v) {
    try {
        return mainDocument.pages[getNewestPage()].textFrames[0].geometricBounds[2] + spacing
    } catch(e) {
        if(mainDocument.pages.length === 1 && mainDocument.pages[0].textFrames.length === 0) {
            if(v === "C") {
                return 24
            }
            return 18
