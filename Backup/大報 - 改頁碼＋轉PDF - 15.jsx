...
///////////    Version 12    ///////////    2021-9-1    ///////////
//getDateMonthYear() to generate the launch date

///////////    Version 13    ///////////    2021-9-2    ///////////
//changed getDateMonthYear() to getIusseDate() to get date from server 

///////////    Version 14    ///////////    2021-9-8    ///////////
//added preflightProcess to preflight() as an argument to fix bug

///////////    Version 15    ///////////    2021-9-28    ///////////
//fixed getIusseDate() bug when the issue is with (Thur.)
...
function main() {
    ...
    if(preflight(preflightProcess) !== false) {
        execute()
    }
}


function preflight(preflightProcess) {
...
//        leftTable.cells.item(1).contents = (getDateMonthYear(new Date().getMonth().toString(), new Date().getDate().toString()))
        leftTable.cells.item(1).contents = issueDate
...
function getIusseDate() {
    const year = new Date().getFullYear()
    var date
    const dateDataFile = app.open(File('//EXZIP18/Gazette_N/_Mis/6221xxxx__' + year + '/Gazette_PageNo & GazIssue/GazIssueDate/GazIssueDate-' + year + '.indd'))
    const table = dateDataFile.textFrames[1].tables[0]

    for(var x = 0; x < table.cells.length; x ++) {
        if(table.cells.item(x).contents.slice(0, 4) === "第" + issue + "期") {
            date = table.cells.item(x + 1).contents
            x = table.cells.length      
        }
    }
    dateDataFile.close(SaveOptions.no)
    return date
}
